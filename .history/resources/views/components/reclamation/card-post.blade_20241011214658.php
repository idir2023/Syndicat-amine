...
@endif
     ...
     style="background-image: url('{{ asset('storage/' . $user->image) }}'); background-size: cover; background-position: center;">
                    ...
                    <div
    class="rounded-[40px] bg-[#F1F1F1] relative m-[0_8px_0_0] p-[12.5px_15px_12.5px_15px] w-full box-sizing-border">
    <input type="text" placeholder="{{__('Contenu de votre commentaire')}}" id="text" name="text"
        class="rounded-[8px] bg-[#F1F1F1] m-[0_120px_0_0] inline-block break-words font-['Inter'] font-normal text-[12px] text-[#A2A2A2] box-sizing-border w-full" />
    <input name="reclamation_id" value="{{ $data->id }}" type="hidden">
</div>
