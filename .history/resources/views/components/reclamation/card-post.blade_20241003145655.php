                        ...
                        <div class="flex flex-col items box-sizing-border w-full b">
                            <div class="m-[0_0_0.5px_0] flex flex-row justify-between w-full box-sizing-border">
                                <span
                                    ...
                                    {{ $comment->user->nom }} “ {{ $comment->user->getRoleNames()->implode(', ') }} ”
                                </span>
                                <div
...
{{-- <div>
        ...
        <!-- Nothing in life is to be feared, it is only to be understood. Now is the time to understand more, so that we may fear less. - Marie Curie -->
            ...
            <div class="rounded-[20px] bg-[#FFFFFF]  relative flex flex-col p-[10px_0_0.5px_0] box-sizing-border box-sizing-border m-1 w-full">
              <div class="m-[0_30px_12.5px_15px] flex flex-row justify-between  box-sizing-border ">

                <div class="m-[7.5px_7.5px_7.5px_0] inline-block break-words font-['Inter'] font-semibold text-[12px] text-[#6F7D93]">
                    <span class="capitalize">{{ $data->type }}</span> : {{ $data->titre }}
                </div>
                <div>
                    <span class="text-right break-words font-['Inter'] font-normal text-[12px] text-[#6F7D93]">
              ...
              <div class="m-[0_30px_11.5px_30px] inline-block self-start break-words font-['Inter'] font-normal text-[12px] text-[#6F7D93]">
                {{ $data->description }}
              </div>
              ...
              @if ($data->commentaires)
                @foreach ($data->commentaires as $comment)
                    <div class="m-[0_30px_0_15px] flex flex-row w-[fit-content] box-sizing-border">
                        <div class="rounded-[100px] m-[0_5px_15.5px_0] w-[30px] h-[30px]"
                            style="background-image: url({{asset($comment->user->image)}})"
                        >
                        </div>
                        <div class="flex flex-col items box-sizing-border">
                            <div class="m-[0_0_0.5px_0] flex flex-row justify-between w-[740px] box-sizing-border">
                                <span class="m-[0_7.5px_0_0] w-[235px] break-words font-['Inter'] font-medium text-[12px] text-[#6F7D93]">
                                {{ $comment->user->nom }} “ {{ $comment->user->getRoleNames()->implode(', ') }} ”
                                </span>
                                <div class="m-[1.5px_0_1.5px_0] inline-block break-words font-['Inter'] font-medium text-[10px] text-[#6F7D93]">
                                    {{ TimeFunction::formatDateToFrench($comment->created_at) }}<br />
                                </div>
                            </div>
                            <span class="m-[0_71.1px_0_0] break-words font-['Inter'] font-normal text-[12px] text-[#6F7D93] text-start	 ">
                                {{ $comment->text }}
                            </span>
                        </div>
                    </div>
              ...
              <form action="{{ route("commentaires.store") }}" method="POST" >
                @csrf
                <div class="m-[0_12px_27px_15px] flex flex-row w-[fit-content] box-sizing-border">
                    <div class="rounded-[50px]  m-[5px_5px_5px_0] w-[30px] h-[30px]"
                        style="background-image: url({{asset($user->image)}})"
                    >
                    </div>
                        <div class="rounded-[40px] border-[1px_solid_#9EAFCE] bg-[#F1F1F1] relative m-[0_8px_0_0] p-[12.5px_15px_12.5px_15px] w-full box-sizing-border">
                            <input
                            type="text"
                            placeholder="Centenu de votre commentaire"
                            id="text"
                            name="text"
                            class="rounded-[8px] border-[1px_solid_#9EAFCE] bg-[#F1F1F1] m-[0_120px_0_0] inline-block break-words font-['Inter'] font-normal text-[12px] text-[#A2A2A2] box-sizing-border w-full"
                        />
