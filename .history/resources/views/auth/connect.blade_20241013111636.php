        ...
        <div class="w-2/3 flex flex-col md:flex-row bg-[#7d8fae] p-10 space-y-8 md:space-y-0 md:space-x-8">
            ...
            <div class="flex-1 flex flex-col items-center text-center md:text-left space-y-6">
                ...
                <p
                    ...
                    {{__('Si vous n`avez pas de compte, remplissez dès maintenant le formulaire dinscription et simplifiez la gestion de votre copropriété') }}
                </p>
        ...
        <div class="w-full md:w-1/3 bg-[#F7F7F7] p-8 flex flex-col justify-center items-center">
            ...
            <form class="space-y-4 w-full max-w-md" action="{{ route('login') }}" method="POST">
                ...
                <div>
                    <label
                        class="block mb-1
                    font-['Inter'] font-medium text-[16px] text-[#6F7D93]">  {{ __('Entrez
                        Votre E-mail ') }}</label>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}"
                                class="break-words font-['Inter'] font-medium text-[16px] text-[#3C4C7C]">{{ __('Mot de passe
                                oublié ?') }}</a>
                        @endif
                    </div>
                    <div class="mb-22 p-20">
                        <button type="submit"
                            class="w-full bg-[#3C4C7C] hover:bg-[#3e569b] text-white
                                py-3 px-6 rounded-full font-bold text-xl">{{ __('Connexion') }}</button>
                    </div>
            ...
            <div class="my-[2.5rem]">
                ...
                @if ($errors->has('residenceInactive'))
                    ...
                    <p class="text-sm text-[#6F7D93]">
                        {{ __('Votre compte résidence est actuellement inactif et vous ne pouvez pas vous connecter. 
                        Veuillez contacter le personnel de votre résidence pour plus d\'informations.') }}
                    </p>
