...
@section('content')
    ...
    <div class="flex-1 w-full ">
        ...
        <div class="grid grid-cols-6 gap-4 mb-10">
            ...
            $roleOrder = ['Super Admin', 'Manager', 'Propriétaire', 'Résident', 'Admin', 'residence'];
        @endphp
        
        @foreach ($roleOrder as $role)
            @if (array_key_exists($role, $roleCounts)) <!-- Check if the role exists in the roleCounts -->
                @if (
                    ...
                    @if ($role !== 'Super Admin')  <!-- Exclude Super Admin from display -->
                        <div class="bg-white p-6 rounded-xl shadow-md text-center transition-transform transform hover:scale-105">
                            <p class="text-sm text-gray-500 mb-2">{{ __('' . $role) }}</p>
                            <h2 class="text-3xl font-bold text-indigo-600">{{ $roleCounts[$role] }}</h2>
                        </div>
                    @endif
                @endif
            @endif
