...
{{ $appParameters->app_name ?? '' }}
    ...
    <a href="{{ route('dashboard.residence', $residenceId) }}"
        ...
        <span
            class="m-[12.5px_0_12.5px_0] inline-block break-words font-['Inter'] text-[12px]  @if (Route::currentRouteName() == 'dashboard.index') font-bold text-[#3C4C7C] @else font-normal  @if (Route::currentRouteName() == 'regelement.show') font-bold text-[#3C4C7C] @else font-normal text-[#6F7D93] @endif @endif">
            {{ __('Règlement') }}text-[#6F7D93] @endif">
            {{ __('dashboard') }}
        </span>
    </a>
