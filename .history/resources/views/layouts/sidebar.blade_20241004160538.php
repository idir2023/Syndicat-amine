...
<div
    ...
    <a href="{{ route('document.residence', $residenceId) }}"
        class="m-[0_15px_25px_15px] flex flex-row self-start w-[fit-content] box-sizing-border">
        <div
            class="shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] rounded-[8px] @if (Route::currentRouteName() == 'document.residence') bg-[#cb0b9e] @else bg-[#E9ECEE] @endif relative m-[0_10px_0_0] flex p-[10px_10px_10px_10px] w-[40px] h-[40px] box-sizing-border">
            <div
                class="@if (Route::currentRouteName() == 'document.residence') bg-[url('{{ asset('assets/images/documents_11.png') }}')] @else bg-[url('{{ asset('assets/images/documents_1.png') }}')] @endif bg-[50%_50%] bg-cover bg-no-repeat w-[20px] h-[20px]">
            </div>
        </div>
        <span
