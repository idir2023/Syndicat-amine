...
{{ $appParameters->app_name ?? '' }}
    ...
    <a href="{{ route('dashboard.index') }}"
    class="m-[0_15px_25px_15px] flex flex-row self-start w-[fit-content] box-sizing-border dashboard-link">
     <div
         class="shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] rounded-[8px] @if (Route::currentRouteName() == 'dashboard.index') bg-[#cb0b9e] @else bg-[#E9ECEE] @endif relative m-[0_10px_0_0] flex p-[10px_10px_10px_10px] w-[40px] h-[40px] box-sizing-border dashboard-bg">
         <div
             class="bg-[url('{{ asset('assets/images/dashboard_1.png') }}')] bg-[50%_50%] bg-cover bg-no-repeat w-[20px] h-[20px]">
         </div>
     </div>
     <span
