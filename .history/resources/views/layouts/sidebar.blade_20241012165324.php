...
{{ $appParameters->app_name ?? '' }}
    ...
    <a href="{{ route('dashboard.residence', $residenceId) }}"
        ...
        <div
            ...
            <div
                class="@if (Route::currentRouteName() == 'dashboard.residence') bg-[url('{{ asset('assets/images/dashboard_2.png') }}')] @else bg-[url('{{ asset('assets/images/dashboard_2.png') }}')] @endif bg-[50%_50%] bg-cover bg-no-repeat w-[20px] h-[20px]">
            </div>
        </div>
        <span
