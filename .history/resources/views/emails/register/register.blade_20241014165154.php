        ...
        <div class="absolute top-5 right-5">
            <form action="{{ route('locale.change') }}" method="POST" class="inline-block">
                ...
                <select name="locale" onchange="this.form.submit()"
                    class="border border-gray-300 rounded-md py-1 px-2 bg-white text-gray-700 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent text-sm">
        ...
        <div class="flex items-center justify-center mb-8">
            <div>
                <img id="profileImage" src="{{ asset('assets/images/avatar.png') ?? 'https://via.placeholder.com/60' }}"
                    alt="User Profile" class="h-24 w-24 rounded-full border-2 mr-6">

                <i id="cameraIcon"
                    class="fa-solid fa-camera absolute  bg-gray-200 p-2 rounded-full text-gray-600 cursor-pointer"
            ...
            <div class="text-gray-700 font-['Fredoka_One','Roboto_Condensed'] font-normal text-[14px] leading-[1.786]">
                ...
                @endif
                <p><strong>{{ __('Rôle :') }}</strong> {{ $role }}</p>
                <p><strong>{{ __('Email :') }}</strong> {{ $email }}</p>
            </div>
        </div>

        <form action="{{ route('register.user.store') }}" method="POST" class="flex flex-col space-y-4" enctype="multipart/form-data">
            @csrf
            <!-- Hidden inputs to handle role and email -->
                <input type="file" id="imageUpload" class="hidden" name="profile_image" onchange="previewImage(event)">
