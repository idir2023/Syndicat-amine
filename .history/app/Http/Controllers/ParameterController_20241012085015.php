...
class ParameterController extends Controller
{
        public function updateParameters(Request $request)
    {
        ...
        if ($request->hasFile('lang_file')) {
            // Store the uploaded file in the 'public/json' directory
            $file = $request->file('lang_file');
            $path = $file->store('json', 'public'); // Storing in 'storage/app/public/json'
    
            // Extract the locale from the filename (without the .json extension)
            $locale = pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME);
    
            // Move the uploaded file to the 'resources/lang' folder
            $file->move(resource_path('lang'), $locale . '.json'); // e.g., 'resources/lang/en.json'
    
            // Set the session to use the new locale (optional)
            session(['locale' => $locale]);
    
            // Optionally, you could set the app locale dynamically
            app()->setLocale($locale);
    
            // You can return the path or success response
            return back()->with('success', 'Language file uploaded and locale set.');
        }
