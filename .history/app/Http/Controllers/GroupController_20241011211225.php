    ...
    public function getGroups()
{
    ...
    // Check if any groups exist for the user
    if ($userGroups->isEmpty()) {
        // Return the view with a message if no groups are found
        return view('Chatify::pages.app', [
            'groups' => null, // Pass null to indicate no groups
            'message' => 'Aucun groupe disponible' // Add a message to display in the view
        ]);
    }

    // Return the view with the list of groups
    return view('Chatify::pages.app', [
        'groups' => $userGroups, // Pass the retrieved groups to the view
        'message' => null // No message since groups are available
    ]);
}
