...
class ParameterController extends Controller
{
    ...
    public function updateParameters(Request $request)
{
    // Validate the input, including the logo and JSON file
    $request->validate([
        ...
        'linkedin_link' => 'nullable|url|max:255',
        'instagram_link' => 'nullable|url|max:255',
        'lang_file' => 'nullable|file|mimes:json|max:2048', // JSON file validation
    ]);
    ...
    if ($request->hasFile('lang_file')) {
        $file = $request->file('lang_file');
        $locale = pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME);
        $filePath = $file->storeAs('json_files', $locale . '.json', 'public');

        // Store the file path or file name in the parameter record
        $parameter = Parameter::firstOrCreate();
        $parameter->json_file = $filePath; // Save the file name or path
        $parameter->save();
    ...
    // Logo handling code (same as you had)

    // Save other parameters (app_name, copyright, etc.)
    $parameter = Parameter::firstOrCreate();
    ...
    return back()->with('success', 'Parameters saved');
}
