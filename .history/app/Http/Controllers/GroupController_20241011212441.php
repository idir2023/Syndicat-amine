...
class GroupController extends Controller
{
    ...
    public function getGroups(Request $request)
    {
        ...
        $groups = Group::join('group_users', 'groups.id', '=', 'group_users.group_id')
            ...
            ->select('groups.*')
            ->paginate($request->get('per_page', 10)); // Default to 10 if not provided
        ...
        if (count($groupsList) > 0) {
            ...
            foreach ($groupsList as $group) {
                // Manually build the HTML for each group item
                $groupItems .= '<div class="group-item">';
                $groupItems .= '<h4>' . $group->name . '</h4>'; // Display the group name
                $groupItems .= '<p>' . $group->description . '</p>'; // Display group description
                $groupItems .= '</div>';
            }
        } else {
