 ...
 */
public function changeResidence($residenceId)
{
    // Assuming you want to store it in session for now
    session(['residence_id' => $residenceId]);

    return redirect()->back(); // Redirect back to the previous page
}

public function getGroup(Request $request, $residenceId)
{
    // Get the search input, or default to an empty string
    $search = $request->search ?? '';
    ...
    $users = User::leftJoin('residences', 'users.residence_id', '=', 'residences.id')
            ->where('users.residence_id', $residenceId) // Filter by the specified residence from navbar
            ->where('users.name', 'like', '%' . $search . '%') // Search by user name
            ->where('users.id', '!=', Auth::id()) // Exclude the current user
            ->distinct('users.id') // Ensure unique users
            ->select('users.*') // Select user details
    ...
    // Return the view with users
    return view('tchat.show_group', compact('users', 'residenceId'));
}
