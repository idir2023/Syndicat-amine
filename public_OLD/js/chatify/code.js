...
function cssMediaQueries() {
    ...
    if (window.matchMedia("(max-width: 980px)").matches) {
        ...
        $("body")
            ...
            .find("div")
            .attr("data-action", "1");
        // $("body").find(".groups-list").find("div").attr("data-action", "1");
    } else {
        $("body")
            .find(".messenger-list-item")
            .find("tr[data-action]")
        ...
        $("body")
            ...
            .find("div")
            .attr("data-action", "0");
        // $("body").find(".groups-list").find("div").attr("data-action", "0");
    }
}
...
// Handle payPurchase_due click event
$(document).on("click", "a.payPurchase_due", function (e) {
    e.preventDefault();
            ...
            $("#search").on("keyup", function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        $(".contact-list").html(
                            $(response).find(".contact-list").html()
                        );
                    },
...
$("body").on("click", ".messenger-list-modal", function () {
    // Check if data-action is set to 1
    if ($(this).attr("data-action") == "1") {
        $(".messenger-listView").hide();
    }
    // Retrieve data-contact attribute from the clicked element
    const dataId = $(this).attr("data-contact");
    setMessengerId(dataId); // Set the messenger ID
    IDinfo(dataId); // Call the function with the retrieved ID
    $(".pay_contact_due_modal").addClass("hidden").removeClass("flex");
});

// group

$(document).on("click", "a.getGroup", function (e) {
    e.preventDefault();
    var url = $(this).attr("href");
...
$(document).ready(function () {
    // Fonctionnalité de fermeture de la modale
    $("#closeModal").on("click", function () {
        // Masquer la modale
        $(".groupModal").addClass("hidden").removeClass("flex");
    });
